...
from discord import Asset, Client, Color, Embed, Guild, GuildSticker, HTTPException, Intents, Interaction, Member, Message, Emoji, PartialEmoji, Role, SelectOption, ButtonStyle, User
from discord.app_commands import CommandTree, ContextMenu, AppCommandContext, AppInstallationType, Command, AppCommandError
...
class ExpressionSelect(Select):
    def __init__(self, *, expressions: Sequence[tuple[Expression, ExpressionLocation]], **kwargs):
        super().__init__(options=[SelectOption(label=f":{expression.name}:", emoji=expression if isinstance(expression, (Emoji, PartialEmoji)) else None, description=location.value, value=str(expression.id)) for expression, location in expressions], max_values=min(25, len(expressions)), **kwargs)
        self._expressions = {expression.id: (expression, location) for expression, location in expressions}
...
class GuildSelect(Select):
    def __init__(self, *, guilds: list[Guild], **kwargs):
        super().__init__(options=[SelectOption(label=guild.name, description=guild.description, emoji=random.choice(guild.emojis) if len(guild.emojis) else None, value=str(guild.id)) for guild in guilds], max_values=min(25, len(guilds)), **kwargs)
...
class CopyExpressionsView(View):
    def __init__(self, client: Client, expressions: Sequence[tuple[Expression, ExpressionLocation]], guilds: list[Guild]):
        super().__init__()
        self.client = client
        self.logger = getLogger("discord.expressioncopier")
        self.expression_select = ExpressionSelect(expressions=expressions, placeholder="Select expressions to copy", row=0)
        self.guild_select = GuildSelect(guilds=guilds, placeholder="Select target servers", row=1)
    ...
    async def on_copy(self, interaction: Interaction, button: Button):
        self.stop()
        await interaction.response.defer()
        await (await interaction.original_response()).edit(embed=Embed(color=Color.brand_green(), title="Copying expressions"), view=None)
            ...
            for expression, location in self.expression_select.selected:
                try:
                    if location == ExpressionLocation.STICKER:
                        ...
                        await guild.create_sticker(name=expression.name, description=expression.description, emoji=expression.emoji, file=await expression.to_file(), reason=f"Copying sticker (requested by @{interaction.user.name})")
                    else:
                        await guild.create_custom_emoji(name=expression.name, image=await expression.read(), reason=f"Copying emoji (requested by @{interaction.user.name})")
                except HTTPException as e:
                    self.logger.warning(f"Failed to copy {expression} to {guild}", exc_info=True)
        ...
        if len(failed):
            embeds.append(Embed(
                ...
                description="\n".join(
                    f"- \\:{expression.name}\\: to {guild.name}: {error.text} ({error.code})" for expression, guild, error in failed
                )
            ))
        if not len(embeds):
            embeds.append(Embed(
...
class EmojiCopier(Client):
    ...
    def emojis_in_string(self, string: str):
        return {PartialEmoji.with_state(self._get_state(), id=int(match.group("id")), name=match.group("name"), animated=match.group("anim") is not None) for match in EmojiCopier.EMOJI_REGEX.finditer(string)}
    def reaction_emojis(self, message: Message):
        return {reaction.emoji for reaction in message.reactions if isinstance(reaction.emoji, (Emoji, PartialEmoji)) and reaction.emoji.id is not None}
    ...
    async def extract_expressions(self, interaction: Interaction, message: Message):
        ...
        if len(message.stickers):
            embeds.append(Embed(
                ...
                description="\n".join([f"- [{sticker.name}]({sticker.url})" for sticker in message.stickers])
            ))
        if len(embeds):
    ...
    async def extract_user_assets(self, interaction: Interaction, user: Member | User):
        ...
        embed = Embed(
            ...
            name="Global avatar", value=self.format_asset_link(full_user.avatar) if full_user.avatar is not None else None, inline=False
        )
        ...
        if full_user.banner is not None:
            embed.add_field(name="Banner", value=self.format_asset_link(full_user.banner), inline=False)
        if full_user.avatar_decoration is not None:
            embed.add_field(name="Avatar decoration", value=self.format_asset_link(full_user.avatar_decoration), inline=False)
        if isinstance(user, Member):
            if user.guild_avatar is not None and user.guild_avatar != user.avatar:
                embed.add_field(
                    name="Server avatar", value=self.format_asset_link(user.guild_avatar), inline=False
                )
            if user.display_icon is not None:
                embed.add_field(
                    name="Role icon", value=self.format_asset_link(user.display_icon) if isinstance(user.display_icon, Asset) else user.display_icon, inline=False
                )
    ...
    async def extract_server_assets(self, interaction: Interaction):
        ...
        if guild is None:
            ...
            embed = Embed(
                ...
                name="Icon", value=self.format_asset_link(guild.icon) if guild.icon is not None else None, inline=False
            )
            embed.set_thumbnail(url=guild.icon.url if guild.icon is not None else None)
            if guild.banner is not None:
                embed.add_field(name="Banner", value=self.format_asset_link(guild.banner), inline=False)
            if guild.splash is not None:
                embed.add_field(name="Invite splash", value=self.format_asset_link(guild.splash), inline=False)
            if guild.discovery_splash is not None:
                embed.add_field(name="Discovery splash", value=self.format_asset_link(guild.discovery_splash), inline=False)
    ...
    async def extract_role_icon(self, interaction: Interaction, role: Role):
        await interaction.response.send_message(embed=Embed(
            color=Color.brand_green(),
            title=f"Assets for role {role.name}"
        ).add_field(
            name="Icon", value=self.format_asset_link(role.display_icon) if isinstance(role.display_icon, Asset) else role.display_icon
        ).add_field(
            name="Color", value=role.color
        ).set_thumbnail(url=role.icon.url if role.icon is not None else None), ephemeral=True)
    ...
    async def copy_expressions(self, interaction: Interaction, message: Message):
        ...
        expressions: list[tuple[Expression, ExpressionLocation]] = list(
            ...
            | {(sticker, ExpressionLocation.STICKER) async for sticker in (await sticker.fetch() for sticker in message.stickers) if isinstance(sticker, GuildSticker)}
        )
        ...
        if not len(expressions):
            ...
            await interaction.response.send_message(view=CopyExpressionsView(self, expressions, elegible_guilds), ephemeral=True)
        else:
    ...
    async def copy_server_expressions(self, interaction: Interaction):
        if interaction.guild is None:
            ...
            expressions = [(emoji, ExpressionLocation.MESSAGE) for emoji in interaction.guild.emojis] + [(sticker, ExpressionLocation.STICKER) for sticker in interaction.guild.stickers]
            elegible_guilds = list(self.elegible_guilds_for_user(interaction.user))
            if not len(expressions):
                await interaction.response.send_message(embed=Embed(
                    color=Color.brand_red(),
